...
const Productedit = () => {
    ...
    useEffect(() => {
        if (!id) return;
            ...
            axios.get(`/collections/${id}`).then(response => {
                ...
                // setAddedPhoto(response.data.photo);
                setfinish(response.data.finishing);
                setYarn(response.data.yarn);
                setWidth1(response.data.width1);
                setWidth2(response.data.width2);
                setWeight(response.data.weight);
                setPrice(response.data.cost);
                setAddedPhoto(response.data.photos);
    ...
    async function submitFabric(e){
        e.preventDefault();
        const data = {id,title,Material,addedPhoto,finish,Yarn,Width1,Width2,Weight,price};
        await axios.put('/clothes',data);
        setRedirect(true)
    }
  ...
  return (
    ...
    {/* content */}
        ...
        <input type="number" placeholder="eg: 5USD" value={price} onChange={e => setPrice(e.target.value)} />
